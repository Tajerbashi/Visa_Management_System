...
{
    public class CallPostApi(string protocol, string port, string controller, string action,string data)
    {
        ...
        public async Task CallApi(string apiKey)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage response = await client.PostAsJsonAsync($"{Controller}/{Action}",Data);
